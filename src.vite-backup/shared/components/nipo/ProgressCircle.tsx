...
export function ProgressCircle({
  ...
  // 🎨 Role-based colors
  const getRoleColor = () => {
    if (!role) return 'stroke-nipo-primary'
  ...
  return (
    <div className={`inline-flex flex-col items-center gap-2 ${getPhilosophyClasses()} ${className}`}>
      {/* 🔄 Progress Circle */}
      <div className="relative">
        ...
        >
          ...
          <circle
            ...
            className={`${getRoleColor()} ${animated ? 'transition-all duration-1000 ease-in-out' : ''}`}
            style={{
...
export function StudentProgressCircle(props: Omit<ProgressCircleProps, 'role'>) {
  return <ProgressCircle {...props} role="student" philosophy="kaizen" />
}

export function ProfessorProgressCircle(props: Omit<ProgressCircleProps, 'role'>) {
  return <ProgressCircle {...props} role="professor" philosophy="zen" />
}

export function AdminProgressCircle(props: Omit<ProgressCircleProps, 'role'>) {
  return <ProgressCircle {...props} role="admin" philosophy="wabi-sabi" />
}