...
type PeriodoRow = Database['public']['Tables']['historia_periodos']['Row']
type CompositorRow = Database['public']['Tables']['historia_compositores']['Row']
type ObraRow = Database['public']['Tables']['historia_obras']['Row']
type EventoTimelineRow = Database['public']['Tables']['historia_eventos_timeline']['Row']

export class HistoriaMusicaService {
  // 📅 PERÍODOS HISTÓRICOS
  // =========================================
  
  static async getAllPeriodos(): Promise<PeriodoRow[]> {
    try {
  ...
  static async getPeriodoById(id: string): Promise<PeriodoRow | null> {
    try {
      const { data, error } = await supabase
        .from('historia_periodos')
        .select('*')
  ...
  static async getAllCompositores(filtros?: {
    ...
    try {
      ...
      if (error) throw new Error(`Erro ao buscar compositores: ${error.message}`)
      return data || []
    } catch (error) {
  ...
  static async getCompositorDetalhes(compositorId: string): Promise<any> {
    try {
      const { data, error } = await supabase
        .from('historia_compositores')
        .select(`
      ...
      if (error) throw new Error(`Erro ao buscar detalhes do compositor: ${error.message}`)
      return data
    } catch (error) {
  ...
  static async getObraDetalhes(obraId: string): Promise<any> {
    try {
      const { data, error } = await supabase
        .from('historia_obras')
        .select(`
      ...
      if (error) throw new Error(`Erro ao buscar detalhes da obra: ${error.message}`)
      return data
    } catch (error) {
  ...
  static async getEventosTimeline(filtros?: {
    ...
    try {
      ...
      if (error) throw new Error(`Erro ao buscar timeline: ${error.message}`)
      return data || []
    } catch (error) {
  ...
  static async buscarConteudo(termo: string, tipos?: string[]): Promise<{
    periodos: PeriodoRow[]
    compositores: CompositorRow[]
    obras: ObraRow[]
  }> {
    try {
  ...
  static async getEstatisticasGerais(): Promise<{
    ...
    distribuicaoPorPeriodo: { periodo: string; compositores: number; obras: number }[]
    timeline: { ano: number; eventos: number }[]
  }> {
    try {
      // Contar totais
      const [
        { count: totalPeriodos },
        { count: totalCompositores },
        { count: totalObras },
        { count: eventosPrincipais }
      ] = await Promise.all([
  ...
  static async getRecomendacoesPeriodo(periodoId: string): Promise<{
    compositoresDestaque: CompositorRow[]
    obrasEssenciais: ObraRow[]
    eventosCruciais: EventoTimelineRow[]
  }> {
    try {
  ...
  static async incrementarPopularidadeObra(obraId: string): Promise<void> {
    try {
      ...
      if (getError) throw new Error(`Erro ao buscar obra: ${getError.message}`)
      ...
      if (error) throw new Error(`Erro ao incrementar popularidade: ${error.message}`)
    } catch (error) {
  ...
  static async getPlaylistPeriodo(periodoId: string, limite = 20): Promise<ObraRow[]> {
    try {
      const { data, error } = await supabase
        .from('historia_obras')
        .select(`
      ...
      if (error) throw new Error(`Erro ao buscar playlist: ${error.message}`)
      return data || []
    } catch (error) {
