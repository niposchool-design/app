...
type DesafioSubmission = Database['public']['Tables']['desafio_submissions']['Row']
type DesafioSubmissionInsert = Database['public']['Tables']['desafio_submissions']['Insert']
...
export class DesafiosService {
   ...
   */
  async getActiveDesafios(): Promise<AlphaDesafio[]> {
    try {
   ...
   */
  async getDesafiosByNivel(nivel: 'facil' | 'medio' | 'dificil'): Promise<AlphaDesafio[]> {
    try {
      const { data, error } = await supabase
        .from('alpha_desafios')
        .select('*')
   ...
   */
  async getDesafioById(desafioId: string): Promise<AlphaDesafio | null> {
    try {
      const { data, error } = await supabase
        .from('alpha_desafios')
        .select('*')
   ...
   */
  async createDesafio(desafioData: AlphaDesafioInsert): Promise<AlphaDesafio | null> {
    try {
      const { data, error } = await supabase
        .from('alpha_desafios')
        .insert({
   ...
   * Atualizar desafio
   */
  async updateDesafio(desafioId: string, updates: AlphaDesafioUpdate): Promise<AlphaDesafio | null> {
    try {
      const { data, error } = await supabase
        .from('alpha_desafios')
        .update(updates)
   ...
   * Desativar desafio
   */
  async deactivateDesafio(desafioId: string): Promise<boolean> {
    try {
   ...
   */
  async submitDesafio(userId: string, desafioId: string, submissionData: any): Promise<DesafioSubmission | null> {
    try {
   ...
   */
  async getUserSubmissions(userId: string): Promise<DesafioSubmission[]> {
    try {
      const { data, error } = await supabase
        .from('desafio_submissions')
        .select(`
   ...
   */
  async getSubmission(userId: string, desafioId: string): Promise<DesafioSubmission | null> {
    try {
      const { data, error } = await supabase
        .from('desafio_submissions')
        .select('*')
   ...
   */
  async hasUserSubmitted(userId: string, desafioId: string): Promise<boolean> {
    try {
   ...
   * Aprovar submissão
   */
  async approveSubmission(submissionId: string, feedback?: string): Promise<boolean> {
    try {
   ...
   * Rejeitar submissão
   */
  async rejectSubmission(submissionId: string, feedback: string): Promise<boolean> {
    try {
      const { error } = await supabase
        .from('desafio_submissions')
        .update({
   ...
   */
  async getPendingSubmissions(): Promise<DesafioSubmission[]> {
    try {
      const { data, error } = await supabase
        .from('desafio_submissions')
        .select(`
   ...
   */
  async getUserDesafioStats(userId: string) {
    try {
