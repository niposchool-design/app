...
export class AchievementsService {
   ...
   */
  async getAchievementsByCategory(categoria: string): Promise<Achievement[]> {
    try {
      const { data, error } = await supabase
        .from('achievements')
        .select('*')
   ...
   */
  async unlockAchievement(userId: string, achievementId: string, progressData?: any): Promise<boolean> {
    try {
   ...
   */
  private async checkLevelUp(userId: string, points: number): Promise<void> {
    try {
   ...
   */
  private async checkLevelAchievements(userId: string, level: string): Promise<void> {
  ...
  async getUserRanking(userId: string): Promise<number | null> {
    try {
      ...
      if (!userProfile?.points) return null
   ...
   */
  async checkAutomaticAchievements(userId: string, action: string, data?: any): Promise<void> {
    try {
   ...
   */
  private async checkSpecificAchievement(userId: string, achievementSlug: string, data?: any): Promise<void> {
