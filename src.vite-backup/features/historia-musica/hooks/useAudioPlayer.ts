...
export const useAudioPlayer = (): UseAudioPlayerReturn => {
  ...
  const [status, setStatus] = useState<StatusAudio>({
    obraAtual: undefined,
    isPlaying: false,
    posicao: 0,
    duracao: 0,
  ...
  const play = useCallback((obra?: ObraDetalhada) => {
    if (!audioRef.current) {
  ...
  const definirVolume = useCallback((volume: number) => {
  ...
  const definirPosicao = useCallback((posicao: number) => {
  ...
  const alternarModoRepeticao = useCallback(() => {
    setStatus(prev => {
      const modos: StatusAudio['modoRepeticao'][] = ['none', 'single', 'playlist']
  ...
  const adicionarPlaylist = useCallback((obras: ObraDetalhada[]) => {
  ...
  const removerDaPlaylist = useCallback((indice: number) => {
    setStatus(prev => {
