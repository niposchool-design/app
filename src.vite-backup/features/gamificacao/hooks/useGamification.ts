...
export function useGamification() {
  ...
  return useQuery({
    ...
    queryFn: async (): Promise<GamificationData> => {
      ...
      const weeklyPoints = weeklyActivity.reduce((sum, activity) => 
        sum + ((activity as any).points || 0), 0
      )
  ...
  return useMutation({
    mutationFn: async ({ points, reason }: { points: number; reason: string }) => {
      ...
      const [pointsRes, activityRes] = await Promise.all([
        (supabase as any).rpc('add_user_points', {
          user_id: user.id,
      ...
      if (pointsRes.error) throw CommonErrors.SUPABASE_ERROR(pointsRes.error.message)
      if (activityRes.error) throw CommonErrors.SUPABASE_ERROR(activityRes.error.message)
