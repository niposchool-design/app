...
export function useInstrumentos() {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      if (error) throw CommonErrors.SUPABASE_ERROR(error.message)
      return data || []
    },
  })
}
...
export function useInstrumentosStats() {
  ...
  const stats = {
    ...
    tempoTotalPratica: meusInstrumentos.data?.reduce((total, item) => 
      total + ((item as any).tempo_pratica_minutos || 0), 0
    ) || 0,
    instrumentosFavoritos: meusInstrumentos.data?.filter(item => 
      (item as any).favorito
    ).length || 0
  }
