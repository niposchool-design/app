...
"[project]/app/api/admin/activity/route.ts [app-route] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
async function GET() {
    try {
        ...
        const { data: profiles } = await supabase.from('profiles').select('id, full_name, tipo_usuario, joined_at').order('joined_at', {
            ascending: false
        }).limit(5);
        // Fetch recent turmas
        const { data: turmas } = await supabase.from('turmas').select('id, nome, criado_em').order('criado_em', {
            ascending: false
        }).limit(5);
        const activities = [
            ...(profiles || []).map((p)=>({
        ...
        return __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$server$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["NextResponse"].json(activities.slice(0, 10));
    } catch (error) {
...
//# sourceMappingURL=%5Broot-of-the-server%5D__6d66811c._.js.map