...
import { getNiveis, getConquistas, getDesafios } from '@/src/lib/supabase/mutations/gamificacao';
...
export default function AdminGamificacaoPage() {
    ...
    useEffect(() => {
        async function loadData() {
            setLoading(true);
            try {
                // Carrega tudo em paralelo (simulado se tabelas nao existirem vai vir vazio)
                const [nData, cData, dData] = await Promise.all([
                    getNiveis(),
                    getConquistas(),
                    getDesafios()
                ]);
                            ...
                            <div className="space-y-6">
                                ...
                                {conquistas.length === 0 ? (
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                                        {conquistas.map(c => (
                                            <div key={c.id} className="bg-white p-6 rounded-xl border border-gray-100 shadow-sm flex flex-col items-center text-center">
                                                ...
                                                <p className="text-sm text-gray-500 my-2 line-clamp-2">{c.descricao}</p>
