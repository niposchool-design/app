...
export default function QRDisplayPage() {
    // useParams return type is generic, we cast or access property
    const params = useParams()
    const id = params?.id as string // This is the 'referencia_id' passed from the manager page
    ...
    const loadQRCode = async () => {
        try {
            // 1. Fetch QR Code
            const { data: qrDataRaw, error } = await supabase
                .from('qr_codes')
                .select('*')
                .eq('referencia_id', id)
                .eq('ativo', true)
            ...
            if (qrDataRaw.tipo === 'aula') {
                const { data: aula } = await supabase
                    .from('aulas')
                    .select('titulo, numero') // Assuming 'numero' exists, if not strictly inferred
                    .eq('id', id)
                    .single()
                if (aula) details = aula
            } else if (qrDataRaw.tipo === 'instrumento') {
                const { data: instrumento } = await supabase
                    .from('instrumentos')
                    .select('nome')
                    .eq('id', id)
                    .single()
                if (instrumento) details = { titulo: instrumento.nome }
            }

            setQrData({ ...qrDataRaw, details })
            setQrCode(qrDataRaw.codigo)
        } catch (error) {
            ...
            <div className="flex flex-col items-center justify-center min-h-[calc(100vh-80px)] p-8">
                ...
                <div className="text-center mb-8">
                    <h1 className="text-4xl font-bold mb-2">
                        {qrData.tipo === 'aula' && qrData.details?.numero ? `Aula ${qrData.details.numero}` :
                            qrData.tipo === 'instrumento' ? 'Instrumento' :
                                'QR Code'}
                    </h1>
                    <p className="text-2xl text-gray-300">
                        {qrData.details?.titulo || qrData.details?.nome || 'Detalhes não disponíveis'}
                    </p>
                </div>
