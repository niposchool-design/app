...
// 🎌 Tipos do sistema de temas japonês
...
export function ThemeProvider({ children }: ThemeProviderProps) {
  ...
  const [mode, setMode] = useState<ThemeMode>(() => {
    if (typeof window !== 'undefined') {
      return (localStorage.getItem('nipo-theme') as ThemeMode) || 'system'
    }
    return 'system'
  })
  ...
  const [userRole, setUserRole] = useState<UserRole>(() => {
    if (typeof window !== 'undefined') {
      return (localStorage.getItem('nipo-user-role') as UserRole) || 'student'
    }
    return 'student'
  })
