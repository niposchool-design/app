...
export const useHistoriaMusica = (): UseHistoriaMusicaReturn => {
  // ========================================
  // 📦 ESTADOS PRINCIPAIS
  // ========================================
  const [periodos, setPeriodos] = useState<PeriodoHistorico[]>([])
  const [compositores, setCompositores] = useState<CompositorDetalhado[]>([])
  const [obras, setObras] = useState<ObraDetalhada[]>([])
  const [eventos, setEventos] = useState<EventoTimelineDetalhado[]>([])
  const [estatisticas, setEstatisticas] = useState<EstatisticasHistoria | null>(null)
  ...
  const carregarCompositores = useCallback(async (filtros?: FiltroCompositores) => {
  ...
  const carregarObras = useCallback(async (filtros?: FiltroObras) => {
  ...
  const carregarTimeline = useCallback(async (filtros?: FiltroTimeline) => {
  ...
  const buscarConteudo = useCallback(async (termo: string): Promise<ResultadoBusca> => {
  ...
  const limparCache = useCallback((chaves?: string[]) => {
    if (!chaves) {
      setCache(new Map())
      return
    }
