...
export class PortfoliosService {
   ...
   */
  async createPortfolio(userId: string, portfolioData: Omit<PortfolioInsert, 'user_id'>): Promise<Portfolio | null> {
    try {
   ...
   * Atualizar portfólio
   */
  async updatePortfolio(portfolioId: string, updates: PortfolioUpdate): Promise<Portfolio | null> {
    try {
      const { data, error } = await supabase
        .from('portfolios')
        .update({
   ...
   */
  async addPortfolioItem(portfolioId: string, itemData: Omit<PortfolioItemInsert, 'portfolio_id'>): Promise<PortfolioItem | null> {
    try {
   ...
   */
  async reorderPortfolioItems(portfolioId: string, itemsOrder: { id: string; ordem: number }[]): Promise<boolean> {
    try {
   ...
   */
  async submitPortfolio(portfolioId: string): Promise<boolean> {
    try {
      const { error } = await supabase
        .from('portfolios')
        .update({
   ...
   */
  async evaluatePortfolio(portfolioId: string, nota: number, feedback: string): Promise<boolean> {
    try {
      const { error } = await supabase
        .from('portfolios')
        .update({
   ...
   */
  async getPortfoliosForReview(): Promise<Portfolio[]> {
    try {
      const { data, error } = await supabase
        .from('portfolios')
        .select(`
