...
export const extractParam = {
  ...
  instrumentSlug: (pathname: string): string | null => {
  ...
  classId: (pathname: string): string | null => {
  ...
  anyParam: (pathname: string, routePattern: string): string | null => {
...
export const routeContext = {
  ...
  getCurrentSection: (pathname: string): 'public' | 'aluno' | 'professor' | 'admin' | 'historia' | 'shared' => {
    if (routeContext.isInAlunoArea(pathname)) return 'aluno'
    if (routeContext.isInProfessorArea(pathname)) return 'professor'
    if (routeContext.isInAdminArea(pathname)) return 'admin'
    if (routeContext.isInHistoriaArea(pathname)) return 'historia'
    if (routeContext.isInProtectedArea(pathname)) return 'shared'
    return 'public'
  }
}
...
export const urlBuilder = {
  ...
  withParams: (baseUrl: string, params: Record<string, string | number | boolean>): string => {
  ...
  withPagination: (baseUrl: string, page: number, limit?: number): string => {
  ...
  withFilters: (baseUrl: string, filters: Record<string, any>): string => {
 ...
 */
export function createNavigationHelpers(currentPath: string) {
  return {
    // Obtém seção atual
    section: routeContext.getCurrentSection(currentPath),
    
    // Verifica se é uma rota específica
    isRoute: (route: string) => currentPath === route,
    
    // Verifica se está em uma seção
    isInSection: (section: string) => currentPath.includes(section),
 ...
 * Testes de navegação
 */
export function testNavigationHelpers(): boolean {
